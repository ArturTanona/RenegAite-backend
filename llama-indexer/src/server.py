...
class DummyNodePostProcessor(BaseNodePostprocessor):
    def _postprocess_nodes(
        ...
        if len(nodes) == 0:
            return [NodeWithScore(node=TextNode(text="No context found"), score=0.0)]  # noqa: E501
        return [nodes[0]]
...
try:
    ...
    aia_index = VectorStoreIndex.from_documents(aia_act, transformations=[splitter], show_progress=True)
...
@app.post("/api/openai/stream")
@app.options("/api/openai/stream")
